...
const MovieDetailPage = () => {
  ...
  const handleSubmitComment = async (event) => {
    ...
    try {
      // ĐỂ ĐỒNG BỘ VỚI STYLE TOAST TRƯỚC ĐÓ CỦA BÁC
      setSubmitting(true);
      const res = await api.post(`/user/movies/${movieId}/comments`, {
        content,
        rating,
      });
      ...
      setCommentContent("");
      setRating(5);
      toast.success("Đã gửi bình luận thành công bác nhé!", {
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-zinc-600 font-black text-xs uppercase tracking-widest">Đang khởi chiếu máy chiếu...</p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans pb-24 relative overflow-hidden animate-fadeIn">
      ...
      >
        <MoveLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> Quay lại
      </button>

      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-24 space-y-16 relative z-10">
        
        {/* ================= PHẦN 1: TỔNG QUAN PHIM (BLOCK VIP) ================= */}
        <div className="flex flex-col lg:flex-row gap-12 items-center lg:items-start bg-zinc-900/20 backdrop-blur-xl p-8 md:p-12 rounded-[40px] border border-zinc-800 shadow-2xl relative overflow-hidden">
          ...
          <div className="w-full max-w-[280px] shrink-0 relative group">
            <div className="absolute inset-0 bg-gradient-to-t from-rose-600 to-transparent opacity-0 group-hover:opacity-20 transition-opacity duration-500 rounded-3xl z-10"></div>
            <img
          ...
          <div className="flex-1 space-y-8 text-center lg:text-left">
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-center lg:justify-start gap-3 text-[11px] font-black tracking-wider text-zinc-400">
                <span className="flex items-center gap-2 bg-zinc-900/80 px-4 py-2 rounded-xl border border-white/5">
                  <Clock size={14} className="text-rose-500" /> {movie.duration} PHÚT
                </span>
                <span className="flex items-center gap-2 bg-zinc-900/80 px-4 py-2 rounded-xl border border-white/5">
                  <Calendar size={14} className="text-rose-500" /> {new Date(movie.releaseDate).getFullYear()}
                </span>
                {comments.length > 0 && (
                  <span className="flex items-center gap-2 bg-zinc-900/80 px-4 py-2 rounded-xl border border-white/5 text-amber-400 shadow-lg shadow-amber-500/5">
                    <Star size={14} className="fill-amber-400 text-amber-400" /> {averageRating} / 5 ({comments.length} Đánh giá)
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-zinc-400 leading-relaxed text-base font-medium max-w-4xl">
                {movie.description || "Nội dung của bộ phim điện ảnh đang được cập nhật, bác vui lòng quay lại sau nhé..."}
              </p>
            </div>
            ...
            <div className="pt-4 flex justify-center lg:justify-start">
              ...
              >
                <Ticket className="group-hover:rotate-12 transition-transform duration-300" size={20} />
          ...
          <div className="lg:col-span-1 bg-zinc-900/30 backdrop-blur-md p-6 md:p-8 rounded-[35px] border border-zinc-800 space-y-6">
            <div>
              <h2 className="text-xl font-black uppercase flex items-center gap-2">
                <MessageCircle className="text-rose-600" size={20} /> Viết Nhận Xét
              </h2>
            ...
            <form onSubmit={handleSubmitComment} className="space-y-5">
              ...
              <div className="bg-zinc-950/60 border border-zinc-800 rounded-2xl p-4 flex items-center justify-between">
                <span className="text-xs font-black text-zinc-400 uppercase tracking-wider">Xếp hạng</span>
                <div className="flex items-center gap-0.5">
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <Star
                        size={20}
                        className={star <= rating ? "fill-amber-400 text-amber-400 shadow-[0_0_10px_rgba(245,158,11,0.5)]" : "text-zinc-700"}
                      />
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-1.5">
                <textarea
                  ...
                  rows={4}
                  maxLength={500}
                  placeholder="Bác thấy bộ phim này thế nào? Chia sẻ cảm nhận tại đây nhé..."
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full flex items-center justify-center gap-3 bg-rose-600 hover:bg-rose-700 disabled:bg-zinc-800 disabled:text-zinc-600 disabled:cursor-not-allowed h-14 rounded-2xl text-xs font-black uppercase tracking-widest transition-all active:scale-[0.98] shadow-lg shadow-rose-600/10"
              >
                <Send size={14} /> {submitting ? "Đang gửi đi..." : "Gửi đánh giá"}
              </button>
            </form>
          </div>

          {/* DÒNG THỜI GIAN CÁC BÌNH LUẬN (PHẢI) */}
              ...
              comments.map((comment) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 pl-2">
                    ...
                    {/* ĐIỂM SAO KHÁCH ĐÁNH GIÁ */}
                    <div className="flex items-center gap-0.5 text-amber-400">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star
                          key={star}
                          size={14}
                          className={star <= comment.rating ? "fill-amber-400 text-amber-400" : "text-zinc-800"}
                        />
                      ))}
                    </div>
                  </div>
                  <p className="text-zinc-400 leading-relaxed text-sm font-medium pl-2">{comment.content}</p>
                </article>
              ))
            )}
          </div>
      ...
      {/* BACKGROUND POSTER KHỔ LỚN BLUR SIÊU ĐẸP */}
      <div className="fixed inset-0 pointer-events-none z-0">
        <img
          src={movie.posterUrl.startsWith("http") ? movie.posterUrl : `https://image.tmdb.org/t/p/original${movie.posterUrl}`}
