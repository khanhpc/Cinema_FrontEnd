...
import AdminEditMoviePage from "./AdminEditMoviePage";
import AdminAddMoviePage from "./AdminAddMoviePage";
...
export default function AdminDashboard() {
  ...
  const handleLogout = () => {
    Swal.fire({
      ...
      customClass: {
        popup: "rounded-[30px] border border-white/10 shadow-2xl backdrop-blur-xl",
      },
    }).then((result) => {
  ...
  const menuItems = [
    { id: "movies", label: "Kho Phim", icon: "M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z" },
    { id: "showtimes", label: "Suất Chiếu", icon: "M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" },
    ...
    { id: "rooms", label: "Phòng Chiếu", icon: "M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" },
    { id: "combo-bongnuoc", label: "Bắp & Nước", icon: "M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" },
    { id: "statistics", label: "Thống Kê", icon: "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v16a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" },
  ];
    ...
    <div className="flex h-screen bg-[#0f111a] text-slate-200 overflow-hidden font-sans">
      ...
      <aside className="w-80 bg-[#161925] border-r border-white/5 flex flex-col relative z-30 shadow-2xl">
        <div className="h-28 flex items-center px-10 border-b border-white/5">
          <div className="flex items-center gap-4 group cursor-pointer" onClick={() => navigate("/")}>
            ...
            <div>
              <h1 className="text-white font-black text-xl leading-none tracking-tighter">CINEMA PLUS</h1>
              <p className="text-[10px] text-rose-500 font-black tracking-[0.4em] uppercase mt-1.5 opacity-80">Hệ thống quản trị</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-6 py-10 space-y-3 overflow-y-auto custom-scroll">
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-5 px-6 py-4 rounded-2xl font-bold text-sm transition-all duration-300 group ${activeTab === item.id
                ? "bg-gradient-to-r from-rose-500 to-rose-600 text-white shadow-xl shadow-rose-500/20 scale-[1.03]"
                : "text-slate-500 hover:bg-white/5 hover:text-white"
                }`}
            >
              <svg className={`w-6 h-6 transition-transform group-hover:rotate-6 ${activeTab === item.id ? "text-white" : "text-slate-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
          ...
          <div className="flex items-center justify-between p-4 bg-white/5 rounded-[25px] border border-white/5 backdrop-blur-md">
            <div className="flex items-center gap-3">
              <div className="relative">
                <img src={`https://ui-avatars.com/api/?name=Admin&background=f43f5e&color=fff`} alt="Admin" className="w-10 h-10 rounded-xl border border-white/10" />
                <span className="absolute -bottom-1 -right-1 w-3 h-3 bg-emerald-500 border-2 border-[#161925] rounded-full"></span>
              </div>
              <div className="min-w-0">
                <p className="text-xs font-black text-white uppercase truncate">KhanhDTK Admin</p>
                <p className="text-[10px] text-slate-500 font-bold uppercase">Chủ rạp</p>
              </div>
            </div>
            <button onClick={handleLogout} className="p-2.5 text-slate-500 hover:text-rose-500 hover:bg-rose-500/10 rounded-xl transition-all active:scale-90" title="Thoát hệ thống">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <header className="h-28 bg-[#0f111a]/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-14 relative z-20">
          <div>
            <p className="text-[10px] font-black text-rose-500 uppercase tracking-[0.4em] mb-2 opacity-80">Bác đang quản lý mục</p>
            <h2 className="text-4xl font-black text-white uppercase tracking-tighter">
              {activeTab === "movies" && isAddingMovie
                ? "Thêm Phim Thủ Công"
                : (activeTab === "movies" && editingMovieId ? "Cập Nhật Bộ Phim" : menuItems.find((i) => i.id === activeTab)?.label)}
            </h2>
          </div>

          <div className="flex items-center gap-6">
            <div className="flex flex-col items-end mr-4">
              <span className="text-xs font-black text-slate-400 uppercase">Hôm nay</span>
              <span className="text-sm font-bold text-white">{new Date().toLocaleDateString("vi-VN")}</span>
            </div>
        ...
        <main className="flex-1 overflow-y-auto p-12 custom-scroll relative z-10 bg-gradient-to-b from-transparent to-black/20">
          <div className="max-w-[1600px] mx-auto animate-pageIn">
            ...
            {activeTab === "movies" && (
              isAddingMovie ? (
                <AdminAddMoviePage onBack={() => setIsAddingMovie(false)} />
              ) : editingMovieId ? (
                <AdminEditMoviePage movieId={editingMovieId} onBack={() => setEditingMovieId(null)} />
              ) : (
                <AdminMoviePage
                  onOpenEdit={(id) => setEditingMovieId(id)}
                  onOpenAdd={() => setIsAddingMovie(true)}
                />
              )
            )}
